...
<form action="{{ route('transaction.store') }}" method="post" class="row" autocomplete="off">
	...
	<div class="col-lg-2 form-group">
		...
		<input type="hidden" name="payment_type" id="transaction-hidden-income" value="income" class="locked" disabled>
		<input type="hidden" name="payment_type" id="transaction-hidden-expense" value="expense" class="locked" disabled>
	</div>
	<div class="col-lg-2 form-group">
...
<form action="{{ route('transaction.index') }}" method="GET" class="form-inline justify-content-center">
	<a href="{{ route('transaction.index', ['date' => $date->subDay()->format('Y-m-d')]) }}" class="btn btn-outline-secondary">&larr; আগের খাতা</a>
	<div class="input-group mx-2">
		...
		<input type="date" name="date" id="old-register-date" class="form-control border-secondary border-left-0" value="{{ old('date', $date->format('Y-m-d')) }}">
	</div>
	<button type="submit" class="btn btn-primary mr-2">দেখুন</button>
	<a href="{{ route('transaction.index', ['date' => $date->addDay()->format('Y-m-d')]) }}" class="btn btn-outline-secondary{{ $date == \Carbon\Carbon::today() ? ' disabled' : '' }}">পরের খাতা &rarr;</a>
</form>
@endsection
