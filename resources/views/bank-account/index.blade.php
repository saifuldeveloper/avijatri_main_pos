...
@section('content')
    <h1>ব্যাংক খাতা <small><a href="{{ route('bank-account.create') }}" class="btn-new" data-toggle="modal"
                data-target="#bank-account-form">নতুন ব্যাংক অ্যাকাউন্ট</a></small></h1>
    ...
    @if ($trashBankAccounts->count() > 0)
        <div class="mt-5">
            <h2> ডিলিট ব্যাংক খাতা </h2>
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach ($trashBankAccounts as $bankAccount)
                        <tr>
                            <td>
                                <a href="{{ route('bank-account.show', ['bank_account' => $bankAccount->id]) }}">
                                    @if ($bankAccount->account_no == 'cash')
    ...
    <div id="bank-account-form" class="modal fade form-modal" tabindex="-1" role="dialog"
