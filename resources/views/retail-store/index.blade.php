...
@section('content')
    <h1>বাকী খাতা <small><a href="{{ route('retail-store.create') }}" class="btn-new" data-toggle="modal"
                data-target="#retail-store-form">নতুন পার্টি</a></small></h1>
    <table class="table table-striped">
        ...
        <tbody>
            @foreach ($regular as $retailStore)
                <tr>
                    <td><a
                            href="{{ route('retail-store.show', ['retail_store' => $retailStore->id]) }}">{{ $retailStore->name }}</a>
                    </td>
    ...
    @if ($trashRetailStores->count() > 0)
        <div class="mt-5">
            <h2> ডিলিট বাকী খাতা / খুচরা পার্টি </h2>
            <table class="table table-striped">
                ...
                <tbody>
                    @foreach ($trashRetailStores as $retailStore)
                        <tr>
    ...
    <div id="retail-store-form" class="modal fade form-modal" tabindex="-1" role="dialog"
