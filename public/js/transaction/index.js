$(document).ready(function() {
	...
	$('#transaction-account-name').change(function(e) {
		...
		var option = $(id).find('option').filter(function() {
	...
	$('#transaction-cheque-no').change(function(e) {
		...
		if($('#transaction-account-type').val() == 'cheque') {
			...
			$.get(url, [], function(data) {
				...
				$('#infobar').html(data.name + ' (' + data.id + '): বাকী = ' + data.due_amount);
...
function unlock() {
	...
	switch(accountType.val()) {
		...
		paymentType.val(accountType.val() == 'withdraw' ? 'income' : 'expense');
...
function loadAccounts() {
	...
	if($(datalistId).length == 0) {
		var datalistUrl = $('#transaction-account-type').attr('data-datalist-url').replace('factory', account);
